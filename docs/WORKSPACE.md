...
It belongs in `infrastructure/.env` (generated by `just env`; never hand-edited
for committed values), pointing at a data root **outside this repo** (e.g.
`~/sentinel-worlds`). **All three must agree** — a split (backend per-world but a
server still shared, or vice versa) writes some state to the wrong tree.
Partial protection exists: a session-create with git-sync still on the shared
tree fails loudly (502, "world provisioning skipped"). The fs-manager case is
**not** auto-detected yet (tracked in `docs/BACKLOG.md`), so verify agreement
before flipping — each server's `/health` now reports `worlds_root`:

```
curl -s localhost:8010/health localhost:8012/health   # fs-manager, git-sync
```

Both must show `"worlds_root": true` once the env is set.
