...
- [2026-06-07, red-team pass] 8 real bugs found across 10 attack surfaces — all `untrusted_llm_path` (schema gate gaps) or loopback-only (MCP destructive ops). Closed-alpha not blocked; strong defenses confirmed (MCP network isolation, world-scoped token-before-lookup, path-traversal-via-id, per-world locks, XFF spoof rejection, namespace-via-query-param gate). → 7 BACKLOG entries filed: schema-gate-tighten (bundled, 4 breaks), teardown_world auth (BEFORE PUBLIC SIGNUP), remove rollback_to, sanitize git error bodies, operator restore-to-prior-turn feature, /api/stream sibling-path fix, community-pack registry.
