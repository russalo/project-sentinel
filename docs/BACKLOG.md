...
- [ ] **ADR 0002 implementation — remaining slices.** Slices 1–4 plus Slice 5's first chunk (resume completeness) have landed. Slice 1 (world_id threaded through backend + git-sync commit messages); Slice 2 (MCP servers resolve a per-world tree under `SENTINEL_WORLDS_ROOT`, UUID-validated + traversal-guarded; engine dispatch + backend threading); Slices 3–5 (below). Teardown + the "my worlds" picker landed (Slice 5); cross-process locking landed (Path A / A1, 2026-06-03 — see sub-item). Remaining: the Slice 5 **provisioning entry point** + residual resume-fidelity follow-ups, and the public-exposure prerequisites (ADR 0003 Slice C edge gate + MCP isolation invariant + systemd — Path A / A2–A3).
...
### Red-team findings — access/isolation audit (2026-06-04)

Construct-and-run red-team Workflow swarm (8 attack surfaces, each fires real hostile requests at a disposable instance; verify-by-repro; every break re-grounded against source). Full triage in gitignored `scratch/review/redteam-2026-06-04-access.md`. The core token gate + traversal guards **held**; below are the confirmed breaks the happy-path "access layer hardened" check missed.

- [ ] **HIGH — `/api/sessions*` leak any world's full transcript, unauthenticated (edge-reachable).** `backend/routes/training.py` `list_sessions` (:48), `get_session` (:91), `export_session` (:117) never call `enforce_world_token` — the sibling `world.py`/`stream.py` routes do. `GET /api/sessions` also enumerates every session unauthenticated, so an attacker needs no prior id to pivot to any world's full narrative + export. **Fix:** the edge template currently proxies **all** `/api/*` (`infrastructure/caddy/Caddyfile.example` `handle /api/*`, :40), so `/api/sessions*` is reachable behind the invite gate **today** — this topology control must be *added*, not just preserved. Add a Caddy `handle /api/sessions*` block that 404s/403s (keeping the training browser tailnet-only), or add an operator-only gate. The route is cross-world by design, so the per-world token doesn't fit; an edge/topology control does. Pairs with the `GET /api/worlds` per-tester-scoping item above.
      _Discovered: 2026-06-04 | Context: red-team finding #1 (sibling-path incompleteness — token gate hardened on /api/world, missed on the parallel /api/sessions* browser routes)._
- [ ] **HIGH — per-IP world-create rate limit bypassable via spoofed `X-Forwarded-For` (edge-reachable; defeats the cost backstop).** `backend/ratelimit.py:104-117` `client_ip()` keys the per-IP create bucket on the *first* `X-Forwarded-For` hop, which a direct caller sets (its docstring even says "not trusted for security"). Rotating the header from one socket peer resets the bucket → unbounded `POST /api/session/new`. **Fix:** key the limiter on the socket peer or the rightmost trusted hop (the one Caddy appends), and have Caddy overwrite inbound XFF — don't key abuse control on a client-controlled header.
      _Discovered: 2026-06-04 | Context: red-team finding #3. The per-world stream limit + LLM ceiling share the same client_ip concern only where keyed on IP._
- [ ] **HIGH — fs-manager protected-field / schema-gate / core-namespace bypass.** `mcp-servers/fs-manager/server.py` `check_protected_fields` (:284) early-returns on non-dict `data` and only inspects top-level keys → a **list-wrapped** or **nested** payload smuggles protected fields (`unique_id`, `namespace`, `world_seed`, `created_at`, `canon`, `core_faction_id`) to disk; `schemas/apply_world_update.schema.json:44` permits `data: ["object","string","array"]` so `validate_payload` doesn't catch it; `validate_namespace` (:311) trusts the payload's self-asserted `namespace` string (the "core authorization token" ARCHITECTURE.md/CLAUDE.md describe is **not** actually enforced) → any caller asserts `namespace:"core"` to write the core tree. **Reachability:** fs-manager is loopback-only (network isolation is the control for *direct* reach), BUT this is the same firewall guarding untrusted DM/LLM `<world_update>` output on the engine→fs-manager path — confirm whether the fact-extractor sanitizes payload *shape* before dispatch. **Fix:** recurse `check_protected_fields` into nested objects + handle list `data` (check each element); tighten the schema/guard so array/nested `data` can't smuggle protected fields; make `namespace:"core"` an actual authorization, not a self-asserted string. **Protected-field smuggling fixed (`fix/redteam-firewall`):** `check_protected_fields` now recurses through dicts + lists, so array-wrapped/nested payloads can't smuggle protected fields. **Remaining → PR 3:** make `namespace:"core"` an out-of-band authorization (trusted backend-set input), not an LLM-payload field.
      _Discovered: 2026-06-04 | Context: red-team findings #5/#6/#7 — the documented "schema-gate bypass" + "protected fields immutable" invariants, broken. **Independently confirmed 2026-06-05 by a Gemini Maestro code-audit (convergence across the construct-and-run + code-reading layers → strongest real-bug signal).** Maestro's fix-refinement: determine `namespace` out-of-band (trusted identity / `X-Sentinel-Namespace` header), not a payload field the LLM sets._
- [ ] **LOW–MED — fs-manager malformed-payload uncaught 500 + provider-error info leak.** (a) `server.py:486` logs `payload.get(...)` *before* `validate_payload` at `:491`, and the route try/except catches only `OSError` → a non-dict/malformed body (list/str/int/null/invalid-JSON) raises an uncaught `AttributeError` → bare 500 + server-side stack trace instead of the structured 422 (the documented "non-dict world_update" pattern). Fix: `isinstance(payload, dict)` guard returning a structured 422 before the log line. (b) `backend/routes/session.py:153-157` (502 body) and `stream.py:160-161` (SSE error event) pass the raw upstream `openai`-SDK error string (org id, quota) to the client. Fix: log server-side, return a generic message. **Both fixed:** (a) the non-dict/invalid-JSON 500 → structured 422 (`fix/redteam-firewall`); (b) the provider-error leak (PR #92, `fix/redteam-edge-highs`).
      _Discovered: 2026-06-04 | Context: red-team findings #2/#8 (malformed-input robustness) and #4 (provider-error info disclosure)._
