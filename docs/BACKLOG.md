...
- [ ] **Lorekeeper / world-lore retrieval (RAG step in the turn loop).** REDIRECTED 2026-06-29 (was "Lorekeeper agent + ChromaDB indexing"): **vendor the `trellis` engine, NOT ChromaDB vector RAG.** trellis (deterministic md/JSONL → Node+Edge graph + query) was probe-proven against real Sentinel lore — determinism + citable provenance + per-world (corpus=folder) scoping fit Sentinel's invariants where vector RAG fights them (ChromaDB is currently `stopped`). Built as a **dedicated-session spinoff** (per the operating model): the session owns the lore→corpus adapter (entity JSON → JSONL `type`/`id` + a `.trellis.yaml` `group:` for currentLocation/role/faction…) + the context-assembly query recipes (location-traversal + canon-FTS, threading `@source` for citation); Sentinel authors the corpus contract + folds the query results into the turn loop. Companion: ratify an **ADR** ("retrieval substrate: vendor trellis, retire ChromaDB-RAG") when the work spins up. Spec + handoff brief: `scratch/ecosystem-substrate-rfc-draft.md` + `scratch/lorekeeper-handoff-brief.md` (local working drafts). trellis grounding confirmed with trellis Claude (file-based, no streaming; pin `trellis >= 0.19.1`). **UPDATE 2026-07-01: the spinoff is `poggio` (`russalo/poggio`, at `senlab/poggio`), v0.1.0 SHIPPED + verified.** Sentinel exercised all 3 recipes live against the sample world (`poggio query --world <root-containing-data/> --recipe at-location|members|established`); the contract holds (dir→kind, filename-stem→id, name→slug refs, per-turn mentions from the blessed `sessions/*.json` — see `[[project_poggio_session_json_softcontract]]`), provenance is first-class, output = ranked `{id,kind,source,snippet|attrs}`. **UPDATE 2026-07-01: the fold Slice 1 LANDED — RFC-0011 + ADR-0006.** `engine/agents/lorekeeper.py` (pure render) + `backend/state/lorekeeper.py` (poggio subprocess, IO) + `DMTurnInput.retrieved_lore` + the `stream.py` wiring, gated by `SENTINEL_LOREKEEPER_ENABLED` (**dormant by default**) and **fail-open** (missing/broken poggio → no canon, never breaks a turn). Slice 1 recipes: `at-location` + `established`; lean projection consumer-side. Verified end-to-end against the sample world (real poggio) + dormant/fail-open paths. **UPDATE 2026-07-01: Slice 2 LANDED — RFC-0012.** Adopted poggio **v0.2.0**'s lean output directly (`_project` → validate + pass-through, consumer-side truncation removed) + a cached `poggio schema-version` assert (expected `0.2`; wrong/old binary → retrieval disabled for the process, fail-open loud). Verified end-to-end against real poggio v0.2.0. Pin **`poggio >= 0.2.0`**. Still dormant. **UPDATE 2026-07-01: ARMED + LIVE.** Marker-gated arm (PR #168: `~/.sentinel-lorekeeper-armed` + `just env` + backend restart) executed 2026-07-01 23:46 UTC; behaviorally verified against a real live world (retrieval fired, canon hits returned, zero fail-open warnings); `serves`=ACTIVE relayed to Blueprint. Rollback: `rm` the marker + `just env` + restart. The same restart put RFC-0010 (time+rest) live. **Remaining:** (b) **Slice 3** — the `members` recipe (focal-NPC neighborhood; deterministic faction-of-in-scene-NPCs focal-selection + budget re-balance 4/2/2); (c) a UI surface for cited canon (later). Channel: build → `russalo/poggio` issues (`from:sentinel`); design/coordination → via Russell.
...
- [x] **Stale `POSTGRES_*` vars in the live `infrastructure/.env` (NOT the template).** ✅ RESOLVED 2026-06-25 (PR #156). The template was always Postgres-free; the fix was operational — re-run `just env` on origin-core. The chezmoi env reconcile did exactly that, regenerating `.env` from the (now correctly-pathed, host-gated) template and dropping the stale `POSTGRES_*` + `DB_VECTOR_PORT` leftovers. (The original 2026-06-03 "emitted by the template" claim was a verify-against-disk miss — it was the stale *generated* file, not the template.) Side note retained: `CHROMA_AUTH_*` are emitted empty → ChromaDB runs unauthenticated (loopback-only; a public-exposure note, not a rotation secret).
...
- [ ] **World identity, world_seed persistence, and multi-session semantics.** Three related gaps, all of which should be settled together in one ADR (smaller scope than 0001 but same shape), because any partial answer creates more confusion than no answer.
  ...
  **Before implementation, write an ADR.** It should cover: the one-world-per-clone decision (or a decision to support multiplexing), the `genesis` block schema, the commit message format change, and the frontend UX for resume/new-session-in-world. Probably 200–400 lines, roughly half the length of ADR 0001. **→ Drafted as [ADR 0002](adr/0002-world-identity-and-isolation.md) (Proposed, 2026-06-02)** — recommends repo-per-world isolation, `world_id`/genesis, per-world write locking, and `/w/<world_id>` routing, to reach concurrent isolated worlds for public test users. Auth/public-exposure deferred to a follow-on ADR (0003).
      ...
      _Closed 2026-06-22: The session-boundary bug is now resolved end-to-end. ADR 0002 Slices 1–5 have shipped, the operational cutover (Slice 6 / Path A / A4) is armed on origin-core (`SENTINEL_WORLDS_ROOT=<WORLDS_ROOT>`), and `backend/routes/session.py::new_session` mints a fresh `world_id` + provisions a clean per-world repo via `engine.init_world` on every create. No cross-world state can bleed because each world's `data/` tree lives in its own directory + git repo. The smoke-test Critical item is closed; the ADR 0002 implementation item below stays open for its residual Slice 5 resume-fidelity follow-ups._
...
**Landed so far (the nine-module default set):** RFC-0005 (module infra)
· RFC-0006 (d100 resolution + four-stat sheet) · RFC-0007 (class + combat,
HP pool + death saves) · RFC-0008 (magic, realm pool) · RFC-0009
(progression, milestone level-up) · RFC-0010 (time + rest — day cycle +
short/long rest recovery) · **RFC-0013 (encounter mechanics — five
level-scaled threat tiers, telegraph/surprise reaction checks,
fight/flee/parley/outwit with flee-at-cost, persistent consequences,
non-combat frames; fills the reserved `tension` slot — every
CANONICAL_SUBSYSTEM_ORDER slot is now filled)**. Still open from the
starting list: **weather/environment effects** (partially framed by
RFC-0013's weather encounter frame; ambient rules beyond encounters still
open), **faction/economy basics**, and **death-stakes enforcement** (the
`permadeath` flag is still prompt-only).
...
- [ ] **Extract the shared markdown renderer + codex design tokens into a workspace package.** The public site (`apps/sentinel-site`) copies `minimalMarkdown.jsx` (~528 lines) verbatim from `apps/sentinel-ui` and duplicates the tailwind codex tokens + the Google-Fonts/base CSS. Accepted as the v1 tradeoff (ship the public surface without a refactor), but the two copies will drift — a renderer fix in one won't reach the other. Extract a small shared package (e.g. `packages/codex` for tokens + `packages/markdown` for the renderer) that both apps import; needs a `packages/*` entry in `pnpm-workspace.yaml`. The same drift applies to the 4 tutorial screenshots (`public/guide/*.png`), copied into both apps' `public/` because the shared `TESTER_GUIDE.md` references them — fold these into the shared-asset story.
      _Discovered: 2026-06-30 | Context: building apps/sentinel-site (landing + /guide). The /guide page reuses the in-app tutorial renderer; copy-for-v1 was the approved path, debt captured here._
...
- [x] **Suggested actions as structured field (hybrid: prompt + schema + frontend).** ✅ **Shipped — PR #112 + polish #113.** The DM emits a structured `suggestedActions: [{label, tone}]` array in the `<world_update>` block (plus inline `<action>` tags in the narrative); the frontend renders them as clickable amber pills under the command bar (`ActionPillRail`) that type the label into the input without auto-submitting, supplemented by rule-based always-available actions, with a graceful fallback when the DM omits them. (Kept here, marked done, because the Panel UX item + the Entity Sweeper note below cross-reference it as "#2".) NOTE: the per-tone pill *coloring* shipped then was reverted 2026-06-07 — pills render single-amber until there's a player-visible legend; the `tone` field still round-trips and that re-enablement is tracked separately under "Re-enable DM action-pill tones with a player-visible legend."
      _Discovered: 2026-04-14 | Context: the DM always ends turns with choice-prompts but they're buried in prose; surfacing them as UI commits the player to a cleaner interaction model. Resolved 2026-04 via PR #112/#113._
...
- [ ] **bestiary world_update robustness harness → GitHub-CI gate.** `tests/test_world_update_robustness_bestiary.py` (PR #159, simplified on bestiary v0.10.0 in PR #160) asserts the `<world_update>` parse boundary upholds NEVER-CRASH / NO-ESCAPE / STRUCTURED-REFUSAL / BOUNDED / DETERMINISTIC against bestiary's adversarial corpus. It runs on **origin-core dev** (bestiary `pip install -e`'d) but **`importorskip`s in GitHub CI** — bestiary is a PRIVATE sibling repo and CI's default `actions/checkout` token can't clone it. To make it a real CI gate: add a PAT/deploy-key secret scoped to `russalo/bestiary` + a `pip install git+…bestiary` step in the `lint-python`/test job (a **credentials decision — coordinate with Russell**), OR bestiary publishes where CI can reach. Low urgency: `extract()` rarely changes and the origin-core run covers it.
      _Discovered: 2026-06-29 | Context: bestiary integration (PR #159); the harness is a local/origin-core gate until bestiary is CI-installable._
...
- [x] **Install ADR 0003 Slice C systemd units for the MCP servers on origin-core.** ✅ RESOLVED 2026-06-25. All three services (backend + fs-manager + git-sync) now run as systemd units (`/etc/systemd/system/sentinel-*.service`, `User=<USER>`, `EnvironmentFile=<REPO_ROOT>/infrastructure/.env`), migrated off the tmux-nohup setup. Fixes both pains the item called out: reboot-survival (`WantedBy=multi-user.target`) and clean code-pickup (`systemctl restart` instead of kill+nohup — so the 2026-06-06 stale-code `NAMESPACE_VIOLATION` incident can't recur). `EnvironmentFile=` reads the marker-gated rendered `.env` (PR #156) as root before dropping to `User=`, so the 0600 secret file needs no loosening. Procedure documented in `docs/WORKSPACE.md` § "Per-world isolation cutover". Verified live: all three `active`, posture armed, `just cutover-check` READY, smoke turn served.
...
- [x] **`SENTINEL_TRUSTED_PROXY_HOPS` chezmoi-clobber durable solution.** ✅ RESOLVED 2026-06-25 (PR #156). The chezmoi env reconcile host-gates the entire armed knob set (including `SENTINEL_TRUSTED_PROXY_HOPS=1`) behind a `~/.sentinel-armed` marker file — the generalized form of the proposed per-host conditional. `just env` on origin-core now renders the armed values (and the age-decrypted session secret) and is **safe to run**; dev/unmarked hosts get `=0` (no XFF-spoof regression on boxes without a trusted proxy). The marker is used instead of `.chezmoi.hostname` because the latter resolves to `srv334254` on origin-core (Hostinger's `/etc/hosts` stamp), which would silently disarm the alpha.
...
- [x] **RESOLVED 2026-07-08 (PR `fix/schema-gate-tighten`). (red-team #1, 2026-06-07) Schema-gate tightening — `untrusted_llm_path` failures.** Bundles four related schema-gate breaks all reachable via DM-emitted `<world_update>` → fact-extractor → fs-manager. Schema-valid is not operator-safe today. Decided 2026-06-07: one combined PR (`fix/schema-gate-tighten`), both fact-extractor pre-emit AND fs-manager write-boundary enforcement (defense in depth — fact-extractor for the fast-fail / DM-feedback UX, fs-manager for the universal catch-all that direct-MCP/migration scripts also hit). **Shipped:** schema `if-then` couples operation/extension/data-type + a `#/$defs/noControlChars` guard on `log_entry` & append data; fs-manager `execute_update` mirrors it (422 on append→.json / scalar→.json, `scrub_control_bytes` at both write points, case-folded protected-field check, `json.loads` guarded → `CORRUPTED_STATE`); fact-extractor scrubs `log_entry` pre-emit. 17 regression tests (`tests/fs_manager/test_schema_gate.py` + `tests/test_schema_validation.py`).
