...
- [ ] **ADR 0002 implementation — remaining slices.** Slices 1–3 have landed. Slice 1 (world_id threaded through backend + git-sync commit messages); Slice 2 (MCP servers resolve a per-world tree under `SENTINEL_WORLDS_ROOT`, UUID-validated + traversal-guarded; engine dispatch + backend threading); Slice 3 (below). Remaining: Slices 4–5 + cross-process locking.
    - ✅ **Slice 3 — provisioning + world-aware reads + tracer-soak gate (landed 2026-06-03).** The backend now routes reads per-world (`resolve_world_data_dir` + threaded into `stream.py`/`training.py`/the export script; `StreamRequest` gains `world_id`); worlds are **provisioned** at creation via git-sync `init_world` (`git init` + baseline + initial commit, idempotent, no-op pre-cutover); and the **tracer-soak gate** (`tests/test_world_isolation_tracer_soak.py`) proves zero cross-world leak under concurrency — it caught and drove the fix for a real git-sync cwd race (`repo.index.add` → subprocess `repo.git.add`). Static-shared assets (`schemas/`, presets, core-lore codex) are NOT relocated — they stay read from the shared repo. **The cutover itself is now an operational env flip, not code** (`docs/WORKSPACE.md` § "Per-world isolation cutover"); leaving `SENTINEL_WORLDS_ROOT` unset keeps the shared-tree default. The world-aware reads also resolve the Codex "session reads" finding; the provisioning resolves the gemini "static/mutable split" finding.
