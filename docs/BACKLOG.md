...
- [ ] **Auth strategy decision (future):** two clear paths remaining — (1) simple API key middleware for single-player public deployment, (2) outsourced JWT (Auth0/Clerk/Supabase) if password management is unwanted. SSE streaming endpoint has no conflict with either — auth middleware runs before the stream opens. Decision not needed for 1.0. The Django User model path is no longer on the table now that Django has retired. **→ Decided in [ADR 0003](adr/0003-access-gating-and-public-exposure.md) (Accepted, 2026-06-03)** — for the closed-beta phase: path (1), a single shared invite gate at the Caddy edge (no accounts), plus per-world capability tokens and lenient rate-limiting. Per-user accounts / outsourced JWT (path 2) are explicitly deferred to a future open-signup phase (ADR 0003 § "Out of scope — vision"). The implementation (edge gate, signed session token, rate-limit, MCP network-isolation invariant, systemd units) is unbuilt; it is a hard prerequisite before exposing the mockup URLs publicly.
...
- [ ] **ADR 0002 implementation — remaining slices.** Slice 1 (world_id threaded through backend + git-sync commit messages) and Slice 2 (MCP servers resolve a per-world tree under `SENTINEL_WORLDS_ROOT`, UUID-validated + traversal-guarded; engine dispatch + backend threading) have landed. Remaining:
    - **Slice 3 — cutover + provisioning (HARD GATE).** Setting `SENTINEL_WORLDS_ROOT` is dead-on-arrival until two things land in the *same* slice:
        1. **World provisioning** (`git init` + baseline `data/` tree): git-sync's `get_repo(world_id)` opens `<WORLDS_ROOT>/<world_id>` and raises `InvalidGitRepositoryError` → 500 for any world that was never git-init'd, so fs-manager writes would land on disk but never enter git history (silent audit gap). Provisioning must also settle the **static-shared vs. mutable-per-world split**: JSON `schemas/`, `data/lore/core/presets/`, and authored core-lore codex are read-only shared assets that should *not* be copied into every world's tree — only the mutable state/session tree belongs per-world. Today (Slice 2) only the fs-manager write set relocates; `read_state`, preset loading, and schema loading still read from the shared repo, so the baseline must match that boundary. _(Raised by gemini-code-assist on the doc-refresh PR.)_
        2. **World-aware session reads.** Slice 2 routes session *writes* to the per-world tree, but `backend/routes/stream.py` still reads via `read_session(settings.data_dir, …)` from the shared tree. Today this is consistent (writes also fall back to the shared tree while `WORLDS_ROOT` is unset), but the moment the env is set, a freshly-created per-world session would fail its next `/api/stream` with "Session not found" because the read looks in the wrong tree. The read path (and `load_world_context`) must become world-aware in the cutover. _(Flagged by Codex review on the Slice 2 PR.)_
      Gate the cutover on the tracer-soak harness (stub DM with a per-world token, assert zero cross-world leak, hammer in CI) — prove isolation deterministically, never against a live LLM.
    - **Slice 4 — frontend `/w/<world_id>` routing.**
    - **Slice 5 — world lifecycle** (provisioning entry point, archival, the resume/new-session UX).
    - **Cross-process write locking.** fs-manager's batch update + session-log append (and git-sync's commit) run unguarded; backend, agents, and CLI tools are separate processes, so concurrent writes to one world can interleave or corrupt state. ADR 0002 calls for per-world write locking. **Cross-OS caveat:** the obvious `fcntl.flock` is POSIX-only and would violate this repo's Windows-support constraint — use a portable approach (`filelock` lib, or a `fcntl`/`msvcrt.locking` split) on a `.lock` file inside the world root. Not a Slice 2 regression (writes were already unguarded); slot before public multi-user exposure.
...
- [ ] **git-sync `rollback_to` / `list_snapshots` are world-routable with no auth.** Both endpoints take `world_id` from the request body with no caller-identity binding, and `rollback_to` is a state-destroying op. Once `SENTINEL_WORLDS_ROOT` is set, any caller who can reach git-sync:8012 can target an arbitrary world's repo. This is the access-control gap ADR 0002 explicitly defers. **→ Addressed by [ADR 0003](adr/0003-access-gating-and-public-exposure.md) (Accepted, 2026-06-03):** the resolution is **network isolation, not endpoint auth** — the MCP servers stay bound to `127.0.0.1`/tailnet and are never on the public edge (Caddy only proxies `/api` + `/healthz` to the backend). ADR 0003 makes that a tested invariant (assert non-`0.0.0.0` bind; Caddy must never proxy `:8010`/`:8012`). Optional defense-in-depth (endpoint auth on git-sync) remains available but is not required while the topology holds. Hard prerequisite before exposing the mockup URLs publicly.
