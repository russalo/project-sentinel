...
- [ ] **Adopt file-observer's `provenance` vector for exported training datasets.** When `just export-training-data` produces `datasets/chatlogs/`, also run `fo` against it and stash the manifest alongside the exported corpus. File-observer's per-corpus `provenance` vector (v1.6+) aggregates which models / toolchains / authors produced what content — useful dataset-honesty annotation when the alpha corpus is shared externally (per [[project_minimum_viable_structure_loop]]). Low effort: extend `just export-training-data` to invoke `fo datasets/chatlogs/ > datasets/manifest.json` post-export. **Trigger:** when alpha generates real corpus worth labeling (probably post-cutover). See `/srv/projects/pkplab/scanner/` for file-observer source. Cross-link: [[project_chatlog_home]] for the substrate distinction.
