...
- [ ] **Vestigial Postgres vars in the chezmoi `.env` template.** `infrastructure/.env` still defines `POSTGRES_USER`, `POSTGRES_PASSWORD`, and `POSTGRES_DB`, but Postgres was removed from the stack in ADR 0001 Phase 2 — no code (`backend/`, `engine/`, `mcp-servers/`) references any `POSTGRES_*` var or imports psycopg/sqlalchemy. The vars (and a real-looking password) are emitted by `.chezmoi/dot_infrastructure/dot_env.tmpl` on every `just env`, so a successor could mistake `POSTGRES_PASSWORD` for a live secret. Prune them from the template. While there, sanity-check the other possibly-stale template vars — `CHROMA_AUTH_PROVIDER`/`CHROMA_AUTH_CREDENTIALS_FILE` are emitted empty, so ChromaDB currently runs unauthenticated (a security note for the public-exposure work, not a rotation secret).
      _Discovered: 2026-06-03 | Context: surfaced during a tailnet cert/key-rotation dashboard credential inventory — `POSTGRES_PASSWORD` is present in `infrastructure/.env` but consumed nowhere; CLAUDE.md confirms Phase 2 removed Postgres entirely. The only live external credential is `OPENAI_API_KEY`, which is a LiteLLM-proxy virtual key (→ tailnet gateway at `100.119.83.49:4000`, model qwen3-32b), not a real OpenAI key._
...
- [ ] **ADR 0002 implementation — remaining slices.** Slices 1–4 plus Slice 5's first chunk (resume completeness) have landed. Slice 1 (world_id threaded through backend + git-sync commit messages); Slice 2 (MCP servers resolve a per-world tree under `SENTINEL_WORLDS_ROOT`, UUID-validated + traversal-guarded; engine dispatch + backend threading); Slices 3–5 (below). Remaining: the rest of Slice 5 (provisioning entry point, archival/teardown, "recent worlds" picker + residual resume-fidelity follow-ups) + cross-process locking.
    ...
    - **Cross-process write locking.** fs-manager's batch update + session-log append (and git-sync's commit, and now git-sync's `teardown_world` rmtree) run unguarded; backend, agents, and CLI tools are separate processes, so concurrent writes to one world can interleave or corrupt state — and teardown racing an in-flight `commit_snapshot` for the same world can rmtree the tree mid-commit (delete-underneath-an-active-writer). ADR 0002 calls for per-world write locking. **Cross-OS caveat:** the obvious `fcntl.flock` is POSIX-only and would violate this repo's Windows-support constraint — use a portable approach (`filelock` lib, or a `fcntl`/`msvcrt.locking` split) on a `.lock` file inside the world root. Not a Slice 2 regression (writes were already unguarded); slot before public multi-user exposure.
      _Discovered: 2026-06-03 | Context: gemini-code-assist flagged on the Slice 2 PR; the suggested fcntl fix is non-portable, hence its own tracked item._

- [ ] **Enforce + test the MCP network-isolation invariant (git-sync `rollback_to`/`list_snapshots` have no auth).** Both endpoints take `world_id` from the request body with no caller-identity binding, and `rollback_to` is a state-destroying op; once `SENTINEL_WORLDS_ROOT` is set, any caller who can reach git-sync:8012 can target an arbitrary world's repo. **Decision ([ADR 0003](adr/0003-access-gating-and-public-exposure.md), Accepted 2026-06-03):** resolve this by **network isolation, not endpoint auth** — the MCP servers stay bound to `127.0.0.1`/tailnet and are never on the public edge (Caddy only proxies `/api` + `/healthz`). **Action:** make that a tested invariant — assert fs-manager/git-sync do *not* bind `0.0.0.0`, add a check, and document that Caddy must never proxy `:8010`/`:8012`; optionally add endpoint auth on git-sync as defense-in-depth. Until then the gap is contained only by today's tailnet-internal exposure, so this is a **hard prerequisite before exposing the mockup URLs publicly.**
...
- [ ] **Backend install/run recipes assume an already-activated venv; bare `pip`/`uvicorn` break on PEP 668.** `just install-backend` (`pip install -r backend/requirements.txt`) and `just dev-backend` (`uvicorn …`) call `pip`/`uvicorn` bare, which only works if a Python env with the deps is already on `PATH`. On a stock Ubuntu 24.04 (Python 3.12, marked PEP 668 externally-managed) there's no venv and bare `pip install` is refused, so a fresh or post-reboot backend setup fails until someone manually creates a venv. As of 2026-06-02 the dev backend on origin-core runs from a manually-created `.venv/` at repo root (`.venv/bin/uvicorn …`). Make this reproducible: have `install-backend` create/use `.venv` and `dev-backend` invoke `.venv/bin/uvicorn` (cross-OS: `.venv/bin` vs `.venv/Scripts`), or document the venv-activation step in QUICKSTART/WORKSPACE. Pairs with the requirements-manifest item above.
      _Discovered: 2026-06-02 | Context: surfaced while bringing the backend up on origin-core for the tailnet dev site after a reboot — no venv existed and PEP 668 blocked system pip._
