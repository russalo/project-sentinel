...
- [ ] **HIGH — `/api/sessions*` leak any world's full transcript, unauthenticated (edge-reachable).** `backend/routes/training.py` `list_sessions` (:48), `get_session` (:91), `export_session` (:117) never call `enforce_world_token` — the sibling `world.py`/`stream.py` routes do. `GET /api/sessions` also enumerates every session unauthenticated, so an attacker needs no prior id to pivot to any world's full narrative + export. **Fix:** keep `/api/sessions*` + the `/data` browser off the public edge (Caddy proxies only `session/new`,`stream`,`world`,`healthz`), or add an operator-only gate — the route is cross-world by design, so the per-world token doesn't fit; a topology/edge control does. Pairs with the `GET /api/worlds` per-tester-scoping item above.
