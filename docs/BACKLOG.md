...
- [ ] **HIGH: Close the `ARCHITECTURE.md` ↔ `fs-manager/server.py` spec/code gap.** PR #25 review surfaced that parts of fs-manager's documented enforcement don't exist in code. Two concrete gaps in the current 271-line `server.py`:
  1. **Namespace gate (ARCHITECTURE.md §2).** The spec says writes to `data/{state,lore}/core/` must be blocked unless the payload carries a `"namespace": "core"` authorization token. `server.py` has no namespace-token check anywhere — a community payload that sets `target_file` to `data/state/core/entities/X.json` will succeed. This is load-bearing for the entire "Core vs Community" security story.
  2. **`core_faction_id` protection (ARCHITECTURE.md §4).** The spec's Protected Fields table lists `core_faction_id` as immutable, but `server.py`'s `PROTECTED_FIELDS` set is `{"unique_id", "world_seed", "namespace", "created_at", "canon"}` — no `core_faction_id`.

  **Decision required before implementation:** implement the missing enforcement in `server.py`, OR pare ARCHITECTURE.md §2/§4 back to describe only what actually ships. I strongly lean toward **implement** — the namespace gate is the entire reason the project can claim to safely accept community content, and letting that gap persist is a silent security regression that ADR 0001 assumes doesn't exist. Paring the spec back would mean admitting that the Core/Community split has no teeth today.

  Scope for the fix PR: add a namespace-token check on write paths matching `data/{state,lore}/core/`, add `core_faction_id` to `PROTECTED_FIELDS`, write unit tests for both behaviors as they land. The test work (previously tracked as a separate BACKLOG item) is folded into this one because you can't test a behavior that doesn't exist yet.
      _Discovered: 2026-04-14 | Context: surfaced on PR #25 review — Gemini and Codex both caught that the docs I was correcting still described behavior that server.py doesn't implement. Originally filed as "write fs-manager tests from scratch"; escalated here because the test scope only exists once the spec and code agree_
...
- [ ] **Expand ruff lint scope beyond `mcp-servers/`.** CI's "Lint Python" job runs `ruff check mcp-servers/` and `ruff format --check mcp-servers/` only. `engine/`, `backend/`, and `tests/` are written in Python, have no lint enforcement, and can drift in style without CI catching it. Expand the scope to cover all Python code in the repo and fix whatever lint/format violations the first run surfaces. Likely a small followup PR given the existing Python is already roughly ruff-conformant, but needs verification.
      _Discovered: 2026-04-14 | Context: flagged by Gemini Code Assist on PR #24 while reviewing docs/TESTING.md's "What runs in CI" section_
