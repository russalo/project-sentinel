...
- [ ] **Write the first unit tests for `mcp-servers/fs-manager/`.** The server has zero test coverage today — no `tests/` directory, never has. fs-manager is 299 lines of security-critical path validation, namespace gating, and protected-field enforcement, and it is the entire boundary between the LLM and the filesystem. First slice: path traversal rejection (`..`, absolute paths outside `data/`), protected-field blocklist (`unique_id`, `world_seed`, `namespace`, `created_at`, `canon`, `core_faction_id`), core namespace gate (writes require `"namespace": "core"`), schema validation rejection. Wire into `pytest tests/` once the first tests land. See `docs/TESTING.md` "Near-term test work" item #3 for the full framing.
      _Discovered: 2026-04-14 | Context: originally filed in PR #24 as "wire fs-manager tests into CI" — PR #24-followup corrected that to "write them from scratch" after Codex flagged that the tests/ directory never existed. Also applies to `mcp-servers/git-sync/` which is in the same state_
