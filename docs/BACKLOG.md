...
- [ ] **ADR 0002 implementation — remaining slices.** Slices 1–3 have landed. Slice 1 (world_id threaded through backend + git-sync commit messages); Slice 2 (MCP servers resolve a per-world tree under `SENTINEL_WORLDS_ROOT`, UUID-validated + traversal-guarded; engine dispatch + backend threading); Slice 3 (below). Remaining: Slices 4–5 + cross-process locking.
    ...
    - ◑ **Slice 5 — world lifecycle.** First chunk — *resume completeness* — landed 2026-06-03: the session persists `persona_id`+`mood`, `GET /api/world/<id>` returns them + a `worldState` block, and `/w/<id>` resume restores the persona and rehydrates the world-state panels (not just the narrative). Remaining: provisioning entry point, archival/teardown, a "recent/my worlds" picker + resume/new-session UX. Slice-5-resume follow-ups (deferred): (a) **persona available-mood list** isn't persisted (preset metadata), so resume restores the selected mood + id/name but the mood dropdown shows the store-default options — persist/resolve the persona's moods to fully restore; (b) **`day` is never persisted** (no day field on disk), so a resumed world always shows "Day 1" — pre-existing gap, fix with the genesis/`world_seed` persistence item; (c) **`WorldMetrics` numeric-tension display:** worldStore's tension contract is a string label, but `applyUpdate` (live play) writes the DM hint's numeric tension straight through, so live play already renders a bare number — resume now normalizes via `tensionLabel()`, but the live path should too (or WorldMetrics should accept numeric). (d) **Legacy-mode `worldState` isn't world-scoped:** `GET /api/world/<id>` reads `load_world_context` on the resolved data dir; in per-world mode that's the world's own tree (isolated), but in legacy/shared mode it's the global shared tree, so resuming one world shows the shared panel state. This matches the legacy live path (`stream.py` reads the same shared tree), and the cutover is what delivers true isolation — but in a legacy *multi*-world tree the resumed story (filtered by world_id) and the panels (global) can disagree. _(Codex P1 on the Slice 5 PR; inherent to pre-cutover legacy mode, resolved by setting `SENTINEL_WORLDS_ROOT`.)_
