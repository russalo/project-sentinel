...
- [ ] **LOW–MED — fs-manager malformed-payload uncaught 500 + provider-error info leak.** (a) `server.py:486` logs `payload.get(...)` *before* `validate_payload` at `:491`, and the route try/except catches only `OSError` → a non-dict/malformed body (list/str/int/null/invalid-JSON) raises an uncaught `AttributeError` → bare 500 + server-side stack trace instead of the structured 422 (the documented "non-dict world_update" pattern). Fix: `isinstance(payload, dict)` guard returning a structured 422 before the log line. (b) `backend/routes/session.py:153-157` (502 body) and `stream.py:160-161` (SSE error event) pass the raw upstream `openai`-SDK error string (org id, quota) to the client. Fix: log server-side, return a generic message.
