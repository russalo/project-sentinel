...
- [ADR 0004 — State truthfulness: the engine owns mechanically-determined state](0004-state-truthfulness.md) — _Accepted 2026-07-08. The engine, not the DM, is the source of truth for mechanically-determined state; the DM cannot override a committed mechanical outcome. State is classified engine-owned vs narrative-owned; engine-owned state is enforced either by dispatch-recompute-and-inject (the RFC-0014 death-save pattern — bound the client input, recompute the consequence server-side, inject over the DM) or a write-boundary guard (the red-team #1 protected-field pattern). Prompt instruction is the baseline; fields graduate to hard enforcement as stakes justify. Placement contract: engine computes → backend injects at dispatch → fs-manager guards the write. First proven subsystem: RFC-0014 (death-stakes)._
- [ADR 0005 — Subsystem modularity (swappable roleplay-system modules)](0005-subsystem-modularity.md) — _Accepted 2026-06-23; foundation landed via [RFC 0005](../rfc/0005-module-infrastructure.md). The roleplay engine is a registry of swappable per-subsystem modules; core ships one per subsystem, community can replace any; each world binds a module set at creation (immutable); module-added state is namespaced under `module_data.<subsystem>`; the DM prompt composes from module fragments. (ADR 0004 — state truthfulness — is the sibling track, Accepted 2026-07-08.)_
