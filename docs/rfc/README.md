...
- [RFC 0017 — Progression hard-enforcement (ADR-0004 Slice 1)](0017-progression-enforcement.md) — _opened + **Implemented 2026-07-08**. The first ADR-0004 (state truthfulness) slice: `level` + `stats` are now engine-authoritative, not prompt-honored. A code map showed progression is **mechanism (a)** (recompute-and-inject), not the (b) fs-manager guard the ADR pencilled in — the authorized and attack writes are byte-identical `world_update` ops. `engine/progression.py` computes the growth from the enacted `LevelUpChoice`; the backend injects it at the `stream.py` dispatch seam (deep-merging stored `module_data`), overriding any DM `level`/`stats` write with an SSE notice; the DM prompt drops to propose-and-narrate. PC-scoped; derived hp/magic deferred to Slice 1b (the class HP factor has no code home yet). Mirrors RFC-0014 (death-stakes)._
- [RFC 0016 — Staging as true pre-prod (own backend + world store + mock-DM acceptance harness)](0016-staging-pre-prod.md) — _opened 2026-07-04, **Implemented 2026-07-08**. Carves staging out of prod: its own backend trio (`:8101`/`:8110`/`:8112`) + `~/sentinel-worlds-staging` (token gate off; candidate code via a git worktree), after the shared-backend world-bleed surfaced on the RFC-0015 deploy. `SENTINEL_DM_MODE=mock` (zero LLM) + `tests/fixtures/mock_dm_death_sequence.json`; `just stage-smoke` deploy gate (drives to a verified death); `just stage-candidate <ref>` worktree flow (#177/#178/#179). Brought up + tailnet-repointed 2026-07-08: `sentinel-staging.dev.russalo.com/alpha/api → :8101`, isolation verified (staging worlds land in the staging store, prod untouched)._
...
- [RFC 0006 — Resolution + character-sheet modules](0006-resolution-and-character-sheet.md) — _opened 2026-06-23, implemented 2026-06-23 (two slices). First mechanic modules under [ADR 0005](../adr/0005-subsystem-modularity.md): four-stat 1–10 character sheet (`core/four-stat-v1`, Body/Mind/Heart/Will) + d100 open-ended resolution (`core/d100-open-v1`, `stat×5`, targets 40/60/80/100, margin-driven degree, open-ended 96-00/01-05). DM-requested two-step roll flow: the DM emits a `check_request`, the player clicks to roll a client-side d100 (real randomness), the result resends as a structured `RollResult` and the DM resolves from the margin. Click-to-roll reveal with a static count-up (`CheckRequestRail`). Both modules in `DEFAULT_MODULES` — Sentinel rolls dice on every world. Schema enforcement of `module_data.character_sheet` deferred to its own seam._
- [RFC 0007 — Class + combat modules](0007-class-and-combat.md) — _opened + implemented 2026-06-24. Third mechanic slice under [ADR 0005](../adr/0005-subsystem-modularity.md): four Fantasy classes (`core/four-class-fantasy-v1` — Warrior/Rogue/Mage/Cleric, HP factors 8/6/4/6) + combat (`core/hp-pool-v1`). Combat REUSES the RFC-0006 d100 loop — an attack is a Body check vs the target's Defense (`40 + Body×3 + armor`), round-per-roll. Hybrid damage = weapon die (rolled client-side) + `⌊margin/10⌋`. HP = `Body × class_factor` at `module_data.character_sheet.hp`; PlayerVitals migrated to a fraction-based fill (current/max) with a flat-`health` transition fallback. 0 HP → unconscious; Will-check death-save chain (3 strikes) → dead. All five core modules now in `DEFAULT_MODULES`._
- [RFC 0008 — Magic module](0008-magic.md) — _opened + implemented 2026-06-24. Fourth mechanic slice under [ADR 0005](../adr/0005-subsystem-modularity.md): `core/realm-pool-v1`. Mage/Cleric casting via a `Will×2` pool with tiered costs; 8 realms; caster bindings assigned from fiction (Cleric → one of 4 deity Patrons with domain realms + tenets + `patron_standing`; Mage → one of 4 arcane Traditions with primary + secondary realm). Contested casts reuse the d100 loop (Will check vs DC, margin-scaled effect); uncontested spells just spend pool. Generalized combat's `weapon_die` → `effect_die` so weapons + spells share one magnitude die. Realms/spells/patrons/traditions inlined in the prompt (no per-spell loader v0.1). Sixth core module in `DEFAULT_MODULES`._
- [RFC 0009 — Progression module](0009-progression.md) — _opened + implemented 2026-06-24. Fifth + final v0.1 mechanic slice under [ADR 0005](../adr/0005-subsystem-modularity.md): `core/milestone-v1`. Milestone-based, player-enacted advancement — no XP counter. The DM PROPOSES a level-up at an earned beat by emitting `level_up: {to_level}` and STOPS (the sovereignty wall: the DM never writes `level`/`stats` itself); a `LevelUpCard` (sibling of the RFC-0006 CheckRequestRail) lets the player pick which attribute to raise, and the choice resends so the DM applies exactly that package — `level` → N, chosen stat +1 (cap 10), `hp.max += class_HP_factor` (Warrior 8 / Rogue 6 / Cleric 6 / Mage 4) with `current` following, Will re-derives the caster pool. Levels 1–5. Seventh + final core module in `DEFAULT_MODULES` — the Fantasy core ruleset is complete._
