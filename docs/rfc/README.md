...
- [RFC 0008 — Magic module](0008-magic.md) — _opened + implemented 2026-06-24. Fourth mechanic slice under [ADR 0005](../adr/0005-subsystem-modularity.md): `core/realm-pool-v1`. Mage/Cleric casting via a `Will×2` pool with tiered costs; 8 realms; caster bindings assigned from fiction (Cleric → one of 4 deity Patrons with domain realms + tenets + `patron_standing`; Mage → one of 4 arcane Traditions with primary + secondary realm). Contested casts reuse the d100 loop (Will check vs DC, margin-scaled effect); uncontested spells just spend pool. Generalized combat's `weapon_die` → `effect_die` so weapons + spells share one magnitude die. Realms/spells/patrons/traditions inlined in the prompt (no per-spell loader v0.1). Sixth + final core module in `DEFAULT_MODULES` — the Fantasy core ruleset is complete._
