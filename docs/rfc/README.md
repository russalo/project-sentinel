...
- [RFC 0016 — Staging as true pre-prod (own backend + world store + mock-DM acceptance harness)](0016-staging-pre-prod.md) — _opened 2026-07-04. Carves staging out of prod (its own backend trio + `~/sentinel-worlds-staging`; token gate off; candidate code via worktree) after the shared-backend world-bleed surfaced on the RFC-0015 deploy. **Slice 1 landed:** `SENTINEL_DM_MODE=mock` injects a fixture client into the DM agents (zero LLM cost), replaying a DM-authored death-by-combat sequence (`tests/fixtures/mock_dm_death_sequence.json`) — the deterministic deploy-gate seed. Remaining: the staging trio + systemd units, the `stage-candidate`/`stage-smoke` worktree flow, and the tailnet edge repoint._
