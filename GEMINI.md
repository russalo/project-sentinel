...
- **Design intent: one player per world** — each player gets an *isolated* world.
  ...
  backend cutover config-agreement check) also landed. ADR 0003 **Slice C has
  also landed** (Caddy invite-gate template `infrastructure/caddy/Caddyfile.example`,
  guarded by `tests/test_caddy_invariant.py`; systemd unit templates under
  `infrastructure/systemd/`). The only remaining public-exposure prerequisite
  is the **operational cutover** (arming the `SENTINEL_*` knobs across all
  three services + deploying the edge). Review accordingly: code is written
